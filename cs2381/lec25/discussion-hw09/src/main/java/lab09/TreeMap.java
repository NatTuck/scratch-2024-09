...
public class TreeMap<K extends Comparable<K>, V> {
    ...
    void insert(K key, V val) {
        // for plan B
        var meta = new InsertMeta();
        meta.depth = 0;
        meta.maxDepth = ceil(Math.log2(size());
        meta.needsRebalance = false;

        this.root = this.root.insert(key, val, meta);

        // for plan A
        this.root = this.root.insert(key, val, depth, maxDepth);
        /*
        if (addedItem) {
            this.size += 1;
...
record InsertResult<K extends Comparable<K>, V>(
    BinNode<K, V> node, boolean needRebalance) {
    // pass
}
...
record BinLeaf<K extends Comparable<K>, V>() implements BinNode<K, V> {
    ...
    // PlanB
    public BinNode<K, V> insert1(K key, V val, InsertMeta meta) {

    }

    // Plan A
    @Override
    public InsertResult<K, V> insert(K key, V val, int depth, int maxDepth) {
        // depth += 1;
        // addedItem = true;
        // if (depth > maxDepth) { needsRebalance = true; }
        return new InsertResult<>(
            new BinBranch<K, V>(new Entry<K, V>(key, val), this, this),
            false
        );
    }
